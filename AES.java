//Copyright Popa Tiberiu 2011
//free to use this in any way

//package crypto.assignment.two;

import javax.crypto.Cipher;
import javax.crypto.SecretKey;
import javax.crypto.spec.IvParameterSpec;
import java.security.SecureRandom;
import java.util.Arrays;

public class AES {


    private static int Nb, Nk, Nr;
    private static byte[][] w;

    //Added by Wyatt:
    private static byte[] iv; //fixed-sized input to cryptographic primitives for randomness, starting point

    //our substitution box used in the first step of AES encryption
    private static final int[] sbox = {};

    //our inverse substitution box used in the last step in a decryption round
    private static final int[] inv_sbox = {};

    //
    private static final int[] Rcon = {};

    //the xor function used for our AddRoundKey step in AES round (or in Key Expansion)
    private static byte[] xor_func(byte[] a, byte[] b) {

        //we declare a byte array that is the same size as or original array
        byte[] out = new byte[a.length];

        //we xor each individual bytes of array a and array b with each other
        //and put it in the output array
        for (int i = 0; i < a.length; i++) {
            out[i] = (byte) (a[i] ^ b[i]);
        }
        return out;
    }

    //
    private static byte[][] generateSubkeys(byte[] key) {

        //create our new matrix to generate the keys
        byte[][] tmp = new byte[Nb * (Nr + 1)][4];

        int i = 0;


        //should create a matrix of [Nk][4] size to hold the values from the key
        while (i < Nk) {

            //put the next four letters of the key into the row
            tmp[i][0] = key[i * 4];
            tmp[i][1] = key[i * 4 + 1];
            tmp[i][2] = key[i * 4 + 2];
            tmp[i][3] = key[i * 4 + 3];

            //go to the next row
            i++;
        }

        //i represents the number of 4 byte keys we have now
        i = Nk;


        while (i < Nb * (Nr + 1)) {
            byte[] temp = new byte[4];
            for (int k = 0; k < 4; k++)
                temp[k] = tmp[i - 1][k];
            if (i % Nk == 0) {
                temp = SubWord(rotateWord(temp));
                temp[0] = (byte) (temp[0] ^ (Rcon[i / Nk] & 0xff));
            } else if (Nk > 6 && i % Nk == 4) {
                temp = SubWord(temp);
            }
            tmp[i] = xor_func(tmp[i - Nk], temp);
            i++;
        }

        return tmp;
    }

    private static byte[] SubWord(byte[] in) {
        byte[] tmp = new byte[in.length];

        for (int i = 0; i < tmp.length; i++)
            tmp[i] = (byte) (sbox[in[i] & 0x000000ff] & 0xff);

        return tmp;
    }

    private static byte[] rotateWord(byte[] input) {
        byte[] tmp = new byte[input.length];
        tmp[0] = input[1];
        tmp[1] = input[2];
        tmp[2] = input[3];
        tmp[3] = input[0];

        return tmp;
    }

    private static byte[][] AddRoundKey(byte[][] state, byte[][] w, int round) {

        byte[][] tmp = new byte[state.length][state[0].length];

        for (int c = 0; c < Nb; c++) {
            for (int l = 0; l < 4; l++)
                tmp[l][c] = (byte) (state[l][c] ^ w[round * Nb + c][l]);
        }

        return tmp;
    }

    private static byte[][] SubBytes(byte[][] state) {

        byte[][] tmp = new byte[state.length][state[0].length];
        for (int row = 0; row < 4; row++)
            for (int col = 0; col < Nb; col++)
                tmp[row][col] = (byte) (sbox[(state[row][col] & 0x000000ff)] & 0xff);

        return tmp;
    }

    private static byte[][] InvSubBytes(byte[][] state) {
        for (int row = 0; row < 4; row++)
            for (int col = 0; col < Nb; col++)
                state[row][col] = (byte) (inv_sbox[(state[row][col] & 0x000000ff)] & 0xff);

        return state;
    }

    private static byte[][] ShiftRows(byte[][] state) {

        byte[] t = new byte[4];
        for (int r = 1; r < 4; r++) {
            for (int c = 0; c < Nb; c++)
                t[c] = state[r][(c + r) % Nb];
            for (int c = 0; c < Nb; c++)
                state[r][c] = t[c];
        }

        return state;
    }

    private static byte[][] InvShiftRows(byte[][] state) {
        byte[] t = new byte[4];
        for (int r = 1; r < 4; r++) {
            for (int c = 0; c < Nb; c++)
                t[(c + r) % Nb] = state[r][c];
            for (int c = 0; c < Nb; c++)
                state[r][c] = t[c];
        }
        return state;
    }

    private static byte[][] InvMixColumns(byte[][] s) {
        int[] sp = new int[4];
        byte b02 = (byte) 0x0e, b03 = (byte) 0x0b, b04 = (byte) 0x0d, b05 = (byte) 0x09;
        for (int c = 0; c < 4; c++) {
            sp[0] = FFMul(b02, s[0][c]) ^ FFMul(b03, s[1][c]) ^ FFMul(b04, s[2][c]) ^ FFMul(b05, s[3][c]);
            sp[1] = FFMul(b05, s[0][c]) ^ FFMul(b02, s[1][c]) ^ FFMul(b03, s[2][c]) ^ FFMul(b04, s[3][c]);
            sp[2] = FFMul(b04, s[0][c]) ^ FFMul(b05, s[1][c]) ^ FFMul(b02, s[2][c]) ^ FFMul(b03, s[3][c]);
            sp[3] = FFMul(b03, s[0][c]) ^ FFMul(b04, s[1][c]) ^ FFMul(b05, s[2][c]) ^ FFMul(b02, s[3][c]);
            for (int i = 0; i < 4; i++) s[i][c] = (byte) (sp[i]);
        }

        return s;
    }

    private static byte[][] MixColumns(byte[][] s) {
        int[] sp = new int[4];
        byte b02 = (byte) 0x02, b03 = (byte) 0x03;
        for (int c = 0; c < 4; c++) {
            sp[0] = FFMul(b02, s[0][c]) ^ FFMul(b03, s[1][c]) ^ s[2][c] ^ s[3][c];
            sp[1] = s[0][c] ^ FFMul(b02, s[1][c]) ^ FFMul(b03, s[2][c]) ^ s[3][c];
            sp[2] = s[0][c] ^ s[1][c] ^ FFMul(b02, s[2][c]) ^ FFMul(b03, s[3][c]);
            sp[3] = FFMul(b03, s[0][c]) ^ s[1][c] ^ s[2][c] ^ FFMul(b02, s[3][c]);
            for (int i = 0; i < 4; i++) s[i][c] = (byte) (sp[i]);
        }

        return s;
    }

    public static byte FFMul(byte a, byte b) {
        byte aa = a, bb = b, r = 0, t;
        while (aa != 0) {
            if ((aa & 1) != 0)
                r = (byte) (r ^ bb);
            t = (byte) (bb & 0x80);
            bb = (byte) (bb << 1);
            if (t != 0)
                bb = (byte) (bb ^ 0x1b);
            aa = (byte) ((aa & 0xff) >> 1);
        }
        return r;
    }

    public static byte[] encryptBloc(byte[] in) {
        byte[] tmp = new byte[in.length];


        byte[][] state = new byte[4][Nb];

        for (int i = 0; i < in.length; i++)
            state[i / 4][i % 4] = in[i % 4 * 4 + i / 4];

        state = AddRoundKey(state, w, 0);
        for (int round = 1; round < Nr; round++) {
            //System.out.println(Nr);
            state = SubBytes(state);
            state = ShiftRows(state);
            state = MixColumns(state);
            state = AddRoundKey(state, w, round);
        }
        state = SubBytes(state);
        state = ShiftRows(state);
        state = AddRoundKey(state, w, Nr);

        for (int i = 0; i < tmp.length; i++)
            tmp[i % 4 * 4 + i / 4] = state[i / 4][i % 4];

        return tmp;
    }

    public static byte[] decryptBloc(byte[] in) {
        byte[] tmp = new byte[in.length];

        byte[][] state = new byte[4][Nb];

        for (int i = 0; i < in.length; i++)
            state[i / 4][i % 4] = in[i % 4 * 4 + i / 4];

        state = AddRoundKey(state, w, Nr);
        for (int round = Nr - 1; round >= 1; round--) {
            state = InvSubBytes(state);
            state = InvShiftRows(state);
            state = AddRoundKey(state, w, round);
            state = InvMixColumns(state);

        }
        state = InvSubBytes(state);
        state = InvShiftRows(state);
        state = AddRoundKey(state, w, 0);

        for (int i = 0; i < tmp.length; i++)
            tmp[i % 4 * 4 + i / 4] = state[i / 4][i % 4];

        return tmp;
    }

    public static byte[] encryptECB(byte[] in, byte[] key) {

        Nb = 4;
        Nk = key.length / 4;
        Nr = Nk + 6;


        int length = 0;
        byte[] padding = new byte[1];
        int i;
        length = 16 - in.length % 16;
        padding = new byte[length];
        padding[0] = (byte) 0x80;

        for (i = 1; i < length; i++)
            padding[i] = 0;

        byte[] tmp = new byte[in.length + length];
        byte[] bloc = new byte[16];


        w = generateSubkeys(key);

        int count = 0;

        for (i = 0; i < in.length + length; i++) {
            if (i > 0 && i % 16 == 0) {
                bloc = encryptBloc(bloc);
                System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
            }
            if (i < in.length)
                bloc[i % 16] = in[i];
            else {
                bloc[i % 16] = padding[count % 16];
                count++;
            }
        }
        if (bloc.length == 16) {
            bloc = encryptBloc(bloc);
            System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
        }

        return tmp;
    }

    public static byte[] decryptECB(byte[] in, byte[] key) {
        int i;
        byte[] tmp = new byte[in.length];
        byte[] bloc = new byte[16];


        Nb = 4;
        Nk = key.length / 4;
        Nr = Nk + 6;
        w = generateSubkeys(key);


        for (i = 0; i < in.length; i++) {
            if (i > 0 && i % 16 == 0) {
                bloc = decryptBloc(bloc);
                System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
            }
            bloc[i % 16] = in[i];
        }
        bloc = decryptBloc(bloc);
        System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);


        tmp = deletePadding(tmp);

        return tmp;
    }

    private static byte[] deletePadding(byte[] input) {
        int count = 0;

        int i = input.length - 1;
        while (input[i] == 0) {
            count++;
            i--;
        }

        byte[] tmp = new byte[input.length - count - 1];
        System.arraycopy(input, 0, tmp, 0, tmp.length);
        return tmp;
    }

    //for reduced round.

    public static byte[] encrypt_r(byte[] in, byte[] key) {

        Nb = 4;

        //number of keys is the total (key length / 4)
        Nk = key.length / 4;

        //number of rounds is equal to the number of keys
        Nr = Nk;


        int length;
        byte[] padding;
        int i;

        //determines the length of the padding based off of the length of the input data
        length = 16 - in.length % 16;

        //padding is initialized with how many bytes there are left over to fill
        padding = new byte[length];

        //first character of padding is set to some value
        padding[0] = (byte) 0x80;

        //the rest of the padding is filled in with zeroes
        for (i = 1; i < length; i++)
            padding[i] = 0;

        //new byte array to be used to store the input to be encrypted + padding
        byte[] tmp = new byte[in.length + length];

        //block to store each group of 16 bytes
        byte[] bloc = new byte[16];

        //take in the key value "keyforaesencrypt" and use it to generate subkeys

        //this value was received from our AES_Demo file in the jButton2ActionPerformed method
        w = generateSubkeys(key);

        int count = 0;

        for (i = 0; i < in.length + length; i++) {
            if (i > 0 && i % 16 == 0) {
                bloc = encryptBloc(bloc);
                System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
            }
            if (i < in.length)
                bloc[i % 16] = in[i];
            else {
                bloc[i % 16] = padding[count % 16];
                count++;
            }
        }
        if (bloc.length == 16) {
            bloc = encryptBloc(bloc);
            System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
        }

        return tmp;
    }

    public static byte[] decrypt_r(byte[] in, byte[] key) {
        int i;
        byte[] tmp = new byte[in.length];
        byte[] bloc = new byte[16];


        Nb = 4;
        Nk = key.length / 4;
        Nr = Nk;
        w = generateSubkeys(key);


        for (i = 0; i < in.length; i++) {
            if (i > 0 && i % 16 == 0) {
                bloc = decryptBloc(bloc);
                System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
            }
            bloc[i % 16] = in[i];
        }
        bloc = decryptBloc(bloc);
        System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);


        tmp = deletePadding(tmp);

        return tmp;
    }

    //Added by Wyatt:
    public static void setIV(byte[] iv) {
        AES.iv = Arrays.copyOf(iv, iv.length);
    }

    //CBC Mode:
 /*

Encryption: XOR the current plaintext block with the previous ciphertext block (the IV for the first block) before encrypting.
Decryption: Decrypt the current ciphertext block and then XOR the result with the previous ciphertext block (the IV for the first block).
Because of this difference in processing, separate methods (encryptCBC and decryptCBC) are required for encryption and decryption in CBC mode.


  */

    // CBC Mode Encryption
//        public static byte[] encryptCBC(byte[] plaintext) {
//            if (iv == null) throw new IllegalStateException("IV must be set before encryption.");
//
//            byte[] block = new byte[16];
//            byte[] encrypted = new byte[plaintext.length];
//            byte[] previousBlock = Arrays.copyOf(iv, iv.length);
//
//            for (int i = 0; i < plaintext.length; i += 16) {
//                System.arraycopy(plaintext, i, block, 0, 16);
//                byte[] xorBlock = xor_func(block, previousBlock);
//                byte[] encryptedBlock = encryptBloc(xorBlock);
//                System.arraycopy(encryptedBlock, 0, encrypted, i, 16);
//                previousBlock = Arrays.copyOf(encryptedBlock, encryptedBlock.length);
//            }
//
//            return encrypted;
//        }

    public static byte[] encryptCBC(byte[] plaintext, SecretKey key, IvParameterSpec iv) throws Exception {
        Cipher cipher = Cipher.getInstance("AES/CBC/PKCS5Padding");

        cipher.init(Cipher.ENCRYPT_MODE, key, iv);

        return cipher.doFinal(plaintext);

    }

    public static byte[] decryptCBC(byte[] plaintext, SecretKey key, IvParameterSpec iv) throws Exception {
        Cipher cipher = Cipher.getInstance("AES/CBC/PKCS5Padding");

        cipher.init(Cipher.DECRYPT_MODE, key, iv);

        return cipher.doFinal(plaintext);

    }

    public static byte[] encryptOFB(byte[] plaintext, SecretKey key, IvParameterSpec iv) throws Exception {
        Cipher cipher = Cipher.getInstance("AES_128/OFB/NOPADDING ");

        cipher.init(Cipher.ENCRYPT_MODE, key, iv);

        return cipher.doFinal(plaintext);

    }

    public static byte[] decryptOFB(byte[] plaintext, SecretKey key, IvParameterSpec iv) throws Exception {
        Cipher cipher = Cipher.getInstance("AES_128/OFB/NOPADDING");

        cipher.init(Cipher.DECRYPT_MODE, key, iv);

        return cipher.doFinal(plaintext);
    }
}

//        // CBC Mode Decryption
//        public static byte[] decryptCBC(byte[] ciphertext) {
//            if (iv == null) throw new IllegalStateException("IV must be set before decryption.");
//
//            byte[] block = new byte[16];
//            byte[] decrypted = new byte[ciphertext.length];
//            byte[] previousBlock = Arrays.copyOf(iv, iv.length);
//
//            for (int i = 0; i < ciphertext.length; i += 16) {
//                System.arraycopy(ciphertext, i, block, 0, 16);
//                byte[] decryptedBlock = decryptBloc(block);
//                byte[] xorBlock = xor_func(decryptedBlock, previousBlock);
//                System.arraycopy(xorBlock, 0, decrypted, i, 16);
//                previousBlock = Arrays.copyOf(block, block.length);
//            }
//
//            return decrypted;
//        }

        // OFB Mode Encryption/Decryption

        /*
         *
       OFB mode works by creating a keystream that is independent of the plaintext and ciphertext.

       Process:
       1. Encrypt the initialization vector (IV).
       2. XOR the output with the plaintext to get the ciphertext or XOR it with the ciphertext to get the plaintext.

       Keystream generation in OFB mode is independent of the plaintext and ciphertext
       The encryption and decryption processes are identical. They both involve XORing the data with a generated keystream.
       This symmetry allows us to use a single method

         */
//        public static byte[] encryptDecryptOFB(byte[] input) {
//            if (iv == null) throw new IllegalStateException("IV must be set before using OFB mode.");
//
//            byte[] streamBlock = Arrays.copyOf(iv, iv.length);
//            byte[] output = new byte[input.length];
//
//            for (int i = 0; i < input.length; i += 16) {
//                byte[] encryptedStreamBlock = encryptBloc(streamBlock);
//                System.arraycopy(input, i, streamBlock, 0, Math.min(16, input.length - i));
//                for (int j = 0; j < Math.min(16, input.length - i); j++) {
//                    output[i + j] = (byte) (encryptedStreamBlock[j] ^ streamBlock[j]);
//                }
//                streamBlock = Arrays.copyOf(encryptedStreamBlock, encryptedStreamBlock.length);
//            }
//
//            return output;
//        }
//    }